...
/*  Serial.print(analogRead(locPin));
  Serial.print(" ");
  Serial.print(analogRead(colPin));
  Serial.print(" ");
  Serial.print(analogRead(bPin));
  Serial.print(" ");
  Serial.print(analogRead(progPin));
  Serial.println();*/
...
void autoMode(char *buff)
{
  ...
  else
  {
    ...
    {
  	for (i = 0; i < 8; i++)
  		colorSelect(i, buff[i + 5], autoBright);
    }
    else
...
void manualMode()
{
  ...
  {
  	  //Appropriate colors are created different selections
  	  //This simple logic only accommodates 8 choices
	  if		(setColor == 1) {r = brightness;g = 0;			b = 0;			}
	  else if	(setColor == 2) {r = brightness;g = brightness;	b = 0;			}
	  else if	(setColor == 3) {r = 0;			g = brightness;	b = 0;			}
	  else if	(setColor == 4) {r = 0;			g = brightness;	b = brightness;	}
	  else if	(setColor == 5) {r = 0;			g = 0;			b = brightness;	}
	  else if	(setColor == 6) {r = brightness;g = 0;			b = brightness;	} 
	  else if	(setColor == 7) {r = brightness;g = brightness;	b = brightness;	}
	  else						{r = 0;			g = 0;			b = 0;			}
	  
	  uint32_t color = strip.Color(r,g,b); //Assign the color to the strip
...
void runProg(char index, int brightness)
{
  if(index == 1)
  {
    ...
    colorChase(strip.Color(brightness,0,brightness), 10);	// purple
...
void colorSelect(int seat, char color, int brightness)
{
	int i, r, g, b;
  	//Appropriate colors are created different selections
  	//This simple logic only accommodates 8 choices
	if		(color == 'r') {r = brightness;	g = 0;			b = 0;			}
	else if	(color == 'y') {r = brightness;	g = brightness;	b = 0;			}
	else if	(color == 'g') {r = 0;			g = brightness;	b = 0;			}
	else if	(color == 't') {r = 0;			g = brightness;	b = brightness;	}
	else if	(color == 'b') {r = 0;			g = 0;			b = brightness;	}
	else if	(color == 'p') {r = brightness;	g = 0;			b = brightness;	} 
	else if	(color == 'w') {r = brightness;	g = brightness;	b = brightness;	}
	else				 {r = 0;			g = 0;			b = 0;			}
	
	uint32_t stripColor = strip.Color(r,g,b);	//Assign the color to the strip
